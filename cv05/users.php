...
class UsersDB extends Database {
    ...
    public function save($data)
    {
        ...
        if(!empty($users)){
            ...
            file_put_contents("messages.db", 'Uživatel #' .  $id . ' byl přidán!' . PHP_EOL, FILE_APPEND);
        }
        else{
            ...
            file_put_contents("messages.db", 'Uživatel #1 byl přidán!' . PHP_EOL, FILE_APPEND);
        }
    }
    ...
    public function delete($id)
    {
        ...
        foreach ($users as $user) {
            if($user->id != $id){
                ...
                file_put_contents("messages.db", 'Uživatel #' .  $id . ' byl smazán!' . PHP_EOL, FILE_APPEND);
            }
        }
