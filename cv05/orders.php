...
class OrdersDB extends Database {
    ...
    public function save($data)
    {
        ...
        if(!empty($orders)){
            ...
            file_put_contents("messages.db", 'Objednávka #' .  $id . ' byla přidána!' . PHP_EOL, FILE_APPEND);
        }
        else{
            file_put_contents($this->getPath(), 1 . $this->getDelimiter() . $data['date'] . PHP_EOL, FILE_APPEND);
            file_put_contents("messages.db", 'Objednávka #1 byla přidána!' . PHP_EOL, FILE_APPEND);
        }
    ...
    public function delete($id)
    {
        ...
        foreach ($orders as $order) {
            if($order->id != $id){
                ...
                file_put_contents("messages.db", 'Objednávka #' .  $id . ' byla smazána!' . PHP_EOL, FILE_APPEND);
            }
        }
